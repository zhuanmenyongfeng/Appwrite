...
const JUMP_URL = process.env.JUMP_URL || 'https://nav.eooce.com'; // 跳转地址    
const UUID = process.env.UUID || '88ffae8c-146f-4682-855a-f62339c0cea6'; // UUID
...
const ARGO_DOMAIN = process.env.ARGO_DOMAIN || '';   // 固定隧道域名，留空使用临时隧道     
const ARGO_AUTH = process.env.ARGO_AUTH || '';       // 固定隧道密钥json或token，留空使用临时隧道,json获取地址：https://json.zone.id                        
...
module.exports = async ({ req, res, log, error }) => {
  try {
    ...
    // 根路由 - 301重定向
    if (path === '/' || path === '') {
      return res.redirect(`${JUMP_URL}`, 301);
    }
    ...
    if (path === `/${SUB_PATH}`) {
      try {
        if (fs.existsSync(subPath)) {
          ...
          return res.send(subContent, 200, {
            ...corsHeaders,
            'Content-Type': 'text/plain; charset=utf-8'
          });
        } else {
          return res.json({ error: '订阅内容未生成' }, 404, corsHeaders);
        }
      } catch (err) {
        error('订阅路由错误:', err);
        return res.json({ error: '订阅获取失败' }, 500, corsHeaders);
      }
    }

    // 状态
    if (path === '/status') {
      const shanghaiTime = new Date().toLocaleString('zh-CN', {
    ...
    return res.json({ error: 'Not Found', path: path }, 404, corsHeaders);
